...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({ width: 1920, height: 1080, webPreferences: { nodeIntegration: true, partition: 'persist:icloud-findmy' } });
  ...
  mainWindow.loadURL('https://www.icloud.com/find');
...
function createTray() {
  ...
  tray = new Tray(icon);
  tray.setToolTip('iCloud Find My');
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: 'Show iCloud Find My', click: () => { if (!mainWindow) createWindow(); mainWindow.show(); } },
...
app.on('before-quit', async () => { await session.fromPartition('persist:icloud-findmy').cookies.flushStore(); });
